	...
	<div>
		<ul class="video-list">
          ...
          <img :src="val.imgUrl ? val.imgUrl : defaultImg" alt="" :onerror="defaultImg">
				...
				<div class="layout-item">
					<span v-if="val.status != 0"><i class="iconfont icon-watch"></i> {{val.viewed}}</span>
					<span v-else></span>
	...
	export default {
		...
		methods: {
			play(val){
				...
				window.scrollTo(0, 0);
				// debugger
				if(this.type == 0 && (val.status == 1 || val.status == 0)){ //直播
					if(val.mode == 0){ //http
				...
				}else if(this.type == 0 && val.status == 2){ //点播
...
.status0{
      background: rgb( 255, 65, 123)!important;
    }
    .status1{
      background: rgb( 48, 96, 246)!important;
    }
    .status2{
      background: rgb( 110, 123, 136)!important;
    }
</style>
