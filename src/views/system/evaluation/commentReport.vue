<template>
    ...
    <iframe style="width: 100%;height: 100%" :src="detailInfo.aiReport ? detailInfo.aiReport :''" frameborder="0"></iframe>
    <el-dialog
      title="下载报告"
...
<!--        <el-button v-if="dataList.teacherReport !== null && dataList.teacherReport !== ''" type="text" @click="downloadPDFReport(0)">下载</el-button>-->
        <el-button v-if="dataList.teacherReport !== null && dataList.teacherReport !== ''" type="text" @click="openAiReportNew(0)">查看</el-button>
        <el-button v-if="dataList.teacherReport !== null && dataList.teacherReport !== ''" type="text" @click="openAiReportNew(1)">下载</el-button>
      ...
      <div style="display: flex;justify-content: space-between;align-items: center;padding: 10px 20px;">
        ...
        <el-button v-if="dataList.professionalReport !== null && dataList.professionalReport !== ''" type="text" @click="downloadPDFReport(1)">下载</el-button>
          <span v-else style="width: 180px;">无报告，请联系管理员</span>
      </div>
      <div style="display: flex;justify-content: space-between;align-items: center;padding: 0 20px;margin-bottom: 10px;">
        ...
        <el-button v-if="dataList.bctiReport !== null && dataList.bctiReport !== ''" type="text" @click="downloadPDFReport(2)">下载</el-button>
          <span v-else style="width: 180px;">无报告，请联系管理员</span>
      </div>
...
<script>
	export default {
      ...
      downloadPDFReport(val){
        this.$axios.get('/sm/comment/exportPDFReport', {id: this.$route.query.id,type: 1, form: val},'blob').then(res => {
          let url = window.URL.createObjectURL(new Blob([res]));
					let link = document.createElement('a');
					link.style.display = 'none';
					link.href = url;
					link.download = this.detailInfo.name+'_'+(val == 1 ? '专业版' : val == 0 ? '教师版' : '大数据报告') +'.pdf';
