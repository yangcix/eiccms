	...
	<div style="padding: 40px 0;background-color: white;min-height: 694px;">
        ...
        <svg width="30" height="10" viewBox="0 0 30 10" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <stop offset="1" stop-color="#2F60F6"/>
			...
			<div class="classify">
				<div class="parents-title">学校：</div>
				<ul class="grade">
          <li :class="activeIndex == 0 ? 'act' : ''" @click="changeList(0)"><p class="parent-title">全部</p></li>
					<li v-for="item in labelList" :key="item.orgId" :class="activeIndex == item.orgId ? 'act' : ''" @click="handleOrg(item)">
						{{ item.name }}
					</li>
				</ul>
			</div>
			<div class="classify">
				<div class="parents-title">名师：</div>
				<ul class="grade">
          <li :class="active == 0 ? 'act' : ''" @click="changeItem(0)"><p class="parent-title">全部</p></li>
					<li v-for="item in teacherList" :key="item.userId" :class="active == item.userId ? 'act' : ''" @click="handleTeacher(item)">
						{{ item.name }}
					</li>
				</ul>
			</div>
			...
			<VideoList :videoList="liveList" type="0"></VideoList>
      ...
      getTeacherList() {
        ...
        this.$axios.post('/index/getUser', data).then(res => {
          this.teacherList = res.data;
			...
			handleType() {
				this.$axios.get('/index/getLabelInfo', { categoryId: 7 }).then(res => {
