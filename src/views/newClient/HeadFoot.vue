<template>
  <div style="min-height:937px;position:relative;min-width: 1200px;">
    <!-- 当前为3.5.1.1版本 -->
      ...
      <nav class="newnav">
        <ul class="nav-list">
          <li v-for="(val, index) in navList"
              :key="index"
              :class="{'nav-active': val.route == currentActive || $route.path.includes(val.url) }"
    ...
    <footer>
      <div class="wrap">
        <div class="company-technology">
          ...
          <p>
            <span><a :href="systemInfo.icpRecordUrl !== null ? systemInfo.icpRecordUrl : 'javascript:void(0)'"
            ...
            <span style="margin-left: 10px"><a
                ...
                :target="systemInfo.recordUrl == null ? '' :'_blank'">{{ systemInfo.record }}</a></span>
          </p>
        </div>
      </div>
    </footer>
...
export default {
  ...
  data() {
    return {
      navList: localStorage.getItem('version') == 1 ? [{
        name: '首页',
        route: '/home'
      }, {
        name: '名师课堂',
  ...
  methods: {
    ...
    navTo(val, index) {
      ...
      if (val.route == '/bigData') {
        window.open(val.route, '_blank')
        return;
      }
      this.currentActive = val.route;
      console.log('fffff', this.$route.path)
      this.$router.replace(val.route);
    },
    //设置导航选中
    setNavActive() {
      console.log('dddddd', this.$route.path)
    ...
    updatePwd() {
      ...
      ;
      this.$axios.post('/sys/user/updatePwd', {
  ...
  watch: {
    $route(to, from) {
      // if(to.path !== '/pplay' && to.path !== '/hplay' && to.path !== '/vplay') {
      this.currentActive = this.$route.path;
      // }
    }
  },
