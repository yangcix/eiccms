	...
	<div>
		<ul class="video-list">
          ...
          <img :src="val.imgUrl" alt="" :onerror="defaultImg">
					<!-- <img :src="val.imgUrl" alt="" :onerror="defaultImg"> -->
					<ul class="tag-list" v-if="!val.themeStatus">
				...
				<div class="layout-item" >
					<span v-if="val.themeStatus != 0" style="width: 50px;"><i class="iconfont icon-watch"></i> {{val.viewed}}</span>
					<span v-else></span>
					<span>{{val.nickName ? val.nickName : val.founder}}</span>
				</div>
				<p class="video-status status1" v-if="val.themeStatus == 1">
				...
				<p class="video-status status0" v-if="val.themeStatus == 0"><i class="el-icon-alarm-clock"></i>预告中</p>
				<p class="video-status status2" v-if="val.themeStatus == 2"><i class="el-icon-refresh-left"></i>已结束</p>
