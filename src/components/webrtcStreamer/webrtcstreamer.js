...
var WebRtcStreamer = function WebRtcStreamer (videoElement, srvurl) {
	...
	this.srvurl = srvurl || location.protocol + '//' + location.hostname + ':' + location.port + '/eiccms';
	// console.log(this.srvurl)
	// this.srvurl = `http://lj.eicc.com:8081`
	// console.log(this.srvurl)
	this.pc               = null;    
...
WebRtcStreamer.prototype.connect = function(videourl, audiourl, options, localstream) {
	...
	if (!this.iceServers) {
		...
		axios({url:this.srvurl + "/sm/recordOverview/apiGetIceServers",method:"POST"})
...
WebRtcStreamer.prototype.disconnect = function() {		
	...
	if (this.pc) {
		axios({url:this.srvurl + "/sm/recordOverview/apiHangup", method: "POST", data: {peerid:this.pc.peerid} })
			.then(this._handleHttpErrors)
...
WebRtcStreamer.prototype.onReceiveGetIceServers = function(iceServers, videourl, audiourl, options, stream) {
	...
	try {            
		...
		var callurl = this.srvurl + "/sm/recordOverview/apiCall?peerid="+ this.pc.peerid+"&url="+encodeURIComponent(videourl);
		if (audiourl) {
		...
		this.pc.createOffer(this.mediaConstraints).then(function(sessionDescription) {
			...
			bind.pc.setLocalDescription(sessionDescription
				, function() {
					...
					// axios({url:callurl,method: "POST",data:})
					axios({url:callurl, method: "POST", data:sessionDescription })
...
WebRtcStreamer.prototype.getIceCandidate = function() {
	axios({url:this.srvurl + `/sm/recordOverview/apiGetIceCandidate?peerid=${this.pc.peerid}`, method: "POST" })
		.then(this._handleHttpErrors)
		// .then( (response) => (response.json()) )
...
WebRtcStreamer.prototype.addIceCandidate = function(peerid, candidate) {
	axios({url:this.srvurl + "/sm/recordOverview/apiAddIceCandidate?peerid="+peerid, method: "POST", data: candidate })
		.then(this._handleHttpErrors)
		// .then( (response) => (response.json()) )
