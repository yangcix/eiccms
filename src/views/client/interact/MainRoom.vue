...
export default {
  ...
  mounted() {
    this.getDetail();
    this.getUserInfo();
    this.$bus.off("startInteract", this.start);
    this.$bus.on("startInteract", this.start);
  ...
  methods: {
    ...
    checkLocal(val) {
      if (val) {
        let audioid = JSON.parse(
          sessionStorage.getItem("interactData")
        ).localAudioId;
        let videoid = JSON.parse(
          sessionStorage.getItem("interactData")
        ).localVideoId;
        ...
        console.log(this.localAudioId, "meiy tttttt", this.localVideoId);
    ...
    handleopenMicrophone(data) {
      if (data.method == "connectServer") {
        ...
        if (data.code == 0) {
          ...
          let arr = [];
          let item = {};
          console.log(obj,'objobjobjob>>>>>>>>>>>>>>>>>>>j',this.detail.id)
    ...
    changePosition(val, valItem, e) {
      ...
      // let data = document.getElementById('wrap' + (val == 'mainContent' ? this.me.user_id : valItem.user_id))
      // console.log(data.document,'data')
      // if(data && data.document.fullscreenElement) {
      //   return; // 判断是否有全屏
      // }
