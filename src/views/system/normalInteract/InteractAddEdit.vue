<template>
	<div class="second-wrap">
		<p class="content-title"><el-page-header @back="goBack" :content="$route.query.themeid ? '编辑专递课堂主题页' :'新增专递课堂主题页'"></el-page-header></p>
			...
			<div class="item-scroll">
				<div class="box">
					...
					<div class="item-wrap">
						...
						<div>
							<div class="itemType width-9" style="padding-left:0">
								<el-select @change="handleTheme" v-model="themeValue" :popper-append-to-body="false" placeholder="请选择">
									<el-option
					...
					<div class="item-wrap" v-for="(val, index) in classTimeList" :key="index">
						<p>{{index == 0 ? '课堂时间' : ''}}</p>
							<p :style="{marginRight: index == 0 ? '' : '22px'}"><em>{{index == 0 ? '*' : ''}}</em>{{index == 0 ? '：' : ''}}</p>
					...
					<div >
						<div class="item-wrap">
							<p>主讲教师</p>
							<p><em>*</em>：</p>
							<el-select v-model="mainData.orgId" @change="shcoolChange('0')" placeholder="选择学校" class="width-8" filterable>
								<el-option
						...
						<div class="item-wrap" v-for="(val, index) in assistenList" :key="index">
							<p>{{index == 0 ? '听讲教师' : ''}}</p>
							<p :style="{marginRight: index == 0 ? '' : '20px'}"><em>{{index == 0 ? '*' : ''}}</em>{{index == 0 ? '：' : ''}}</p>
							<el-select v-model="val.orgId" @change="shcoolChange('1', val, index)" placeholder="选择学校" class="width-8" filterable>
								<el-option
									v-for="item in schoolList"
									:key="item.orgId"
									:label="item.name"
					...
					<el-button type="primary" class="edit-btn" v-if="permission.fastRelease" @click="save('release')">一键发布</el-button>
				</div>
				<el-dialog
					:title="title"
	...
	export default {
		...
		data() {
			return {
				...
				addEditInfo: {mode: 1, auditPassword: '', interactivePassword: '', lecturer: '', description: '',groupId: '',recode: 0, joiner: 0, listTeacher: [], lecturerId: '',startTime: '',endTime: ''},
				schoolList: [],
				teacherList: [],
				buildlList: [],
				classroomList: [],
				recodeList: [],
				mainData: {orgId: '', userId: '', buildId: '', classroomId: '', terminalId: '',
				buildlList: '', classroomList: '', terminalList: ''},
		...
		methods: {
			...
			getTypeList(){
				this.$axios.get('/sm/label/listByCategory', {
					...
					treeList.forEach(val => {
						...
						val.value = this.valData.find((v) => v.parentId == val.id) ? this.valData.find((v) => v.parentId == val.id).id : '';
					})
					this.themeTypeList = data;
			...
			saveData() {
				...
				parameters.append('masterList',JSON.stringify([{userId: this.mainData.userId,orgId: this.mainData.orgId}]));
				if(this.themeId) {
			...
			for (let k in dateAr) {
				if (idx !== k) {
						...
						if (dateAr[k].e <= dateAr[idx].s && (this.checkHMS(dateAr[idx].s) - this.checkHMS(dateAr[k].e) <= 60)) {
							return false
						}
				}
			}
			...
			verify(){
				...
				// if (this.addEditInfo.mode === 1) {
					...
					if(this.$verify.letterNum(this.addEditInfo.auditPassword, 20) || this.addEditInfo.auditPassword.length < 6){
