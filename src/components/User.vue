		...
		<div class="main-wrap current-wrap">
			<div class="user-menu">
				<el-menu class="el-menu-demo" mode="horizontal" :default-active="activeIndex" @select="menuSelect">
					<el-menu-item v-for="(val, index) in userList" :index="index + ''">{{val.name}}</el-menu-item>
				</el-menu>

				<div class="tree-list">
					<el-tree :data="treeList" :props="menuProps" @node-click="menuTreeChange">
						<el-tooltip class="item" effect="dark" :content="node.label" placement="top-start" slot-scope="{ node, data }">
			...
			<div class="data-wrap">
				<div class="search-operat" style="min-width: 900px; max-height:40px">
					<div>
						<el-input v-model="searchKey" class="width-2" placeholder="请输入姓名或学号/编号关键字进行查询" v-search="search" clearable></el-input>
					...
					<div>
						...
						<el-button class="width-1" icon="el-icon-delete" @click="mutipleDelete" v-if="permission.delete">删除</el-button>
						<el-upload v-if="permission.importUserInfo"
		           ...
		           :http-request="httpRequest">
							<el-button class="width-1" icon="el-icon-upload2" v-if="permission.importUserInfo">导入</el-button>
						</el-upload>
						<el-button class="width-1" icon="el-icon-download" @click="downLoad" v-if="permission.getExcel">模板</el-button>
					</div>
				</div>
			...
			<el-dialog
				...
				<div class="dialog-wrap">
					...
					<div class="dialog-item">
						<p>班级</p>
						<p><em>*</em>：</p>
						<el-cascader v-model="addEditInfo.classId"
			...
			<el-dialog
				...
				<div class="dialog-wrap">
					...
					<div class="dialog-item">
						<p>部门</p>
						<p><em>*</em>：</p>
						<el-cascader v-model="addEditInfo.deptId"
	             :show-all-levels="false"
	             style="width: 270px"
	             ...
	             clearable>
					...
					<div v-if="activeIndex == 1" class="dialog-item">
						...
						<div class="upload-wrap">
							<el-upload
								...
								<div class="el-upload__text">将图片拖到此处，或<em style="color: #409EFF">点击上传</em></div>
					...
					<div class="dialog-btn">
						<el-button type="primary" @click="otherDddEditConfirm">确 定</el-button>
						<el-button @click="otherAddEditShow = false;fileLists = []">取 消</el-button>
					</div>
				</div>
			</el-dialog>
			...
			<el-dialog
				title="系统提示"
				:close-on-click-modal="false"
				:visible.sync="uploadSuccessShow"
				width="420px">
				<div class="dialog-wrap">
			...
			<el-dialog
				title="系统提示"
				:close-on-click-modal="false"
				:visible.sync="uploadErrorShow"
				width="420px">
				<div class="dialog-wrap">
					<p class="success-icon"><i class="el-icon-warning-outline err-icon"></i></p>
					<p class="success-title">共{{failSize + successSize}}条数据，上传成功{{successSize}}条，异常数据{{failSize}}条！</p>
					<div class="dialog-btn">
			...
			<el-dialog
				...
				<div class="dialog-wrap">
					...
					<div class="dialog-item">
						<p>班级</p>
						<p><em>*</em>：</p>
						<el-cascader v-model="downloadInfo"
						:show-all-levels="false"
						style="width: 270px"
						:options="selectStudentTree"
						placeholder="请选择班级"
	...
	export default {
		...
		data() {
			return {
				...
				addEditStatus: '全部',
				fileList: [],
				userList: [{name: '学生', value: 0},{name: '教师', value: 1},{name: '其他', value: 2}],
		...
		httpRequest(param) {
			...
			verify(){
				if(this.activeIndex == 0 && this.$verify.letterNum(this.addEditInfo.code, 30)){
					this.errorInfo = '学号必填并由大小写字母和数字组成，最长30位！';
					return true;
				}
				if(this.activeIndex != 0 && this.$verify.letterNum(this.addEditInfo.code, 30)){
					this.errorInfo = '编号必填并由大小写字母和数字组成，最长30位！';
					return true;
				}
				if(this.$verify.letterNum(this.addEditInfo.account, 20)){
					this.errorInfo = '账号必填并由大小写字母和数字组成，最长20位！';
					return true;
				}
				if(this.$verify.chLetterNum(this.addEditInfo.nickName, 20)){
					this.errorInfo = '姓名必填并由中文大小写字母数字组成，最长20位！';
					return true;
				}
