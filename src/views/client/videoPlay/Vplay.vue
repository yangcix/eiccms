	...
	<div class="vplayPage">
		<div class="video-content-wrap">
			...
			<div class="video-info">
				...
				<p class="time">
					<span>发布时间：</span>
					<span>{{detailInfo.publishDate}}</span>
				</p>
		...
		<transition name="bounce">
			<div class="evaluation-wrap" v-if="evaluationShow">
				...
				<div class="evaluation-title">{{evaluationList[0].name}}在线评课{{activeName}}--{{evaluationList[activeName].type }}<span style="text-align: right;" @click="openNew">新页面访问</span></div>
        <div class="totalNum">
          <span v-if="evaluationList[activeName].type == 1 || evaluationList[activeName].type == 2">得分：{{totalNum}}</span>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openNew() {
        let type = this.$route.query.resource == 5 ? 2 : 1;
        if (!this.isSubmit) {
          let evaluationData = {
            type: type,
        ...
        let type = '';
        let formData = {};
        if (this.$route.query.resource == 5) {
          type = 2;
          formData = {
          ...
          formData = {
            ...
            userId: this.userId
				...
				this.$axios.post('/index/checkCommentAuth',formData).then(res => {
					if(res.code === 200){
						this.isBindEvaluation = true;
						this.commentData = res.data;
						this.$axios.post('/index/getUserCommentDetail',{type: type,objectId:this.themeId}).then(res => {
          ...
          userId: this.userId
			...
			if (this.commentData.status !== 1) {
                ...
                this.$message(res.message,'error')
						...
						if (this.commentData.status !== 1) {
							...
							this.$axios.post('/index/checkCommentAuth', formData).then(ress => {
								if (ress.code == 200) {
									this.activeName = '0';
			...
			initVideo(){
				this.myVideo = new Player({
					id: 'myVideo',
					// url: `http://192.168.164.59${this.videoList[0].url}`,
					url: `${location.protocol}//${location.host}${this.videoList[0].url}`,
					hasVideo: true,
					hasAudio: true,
					width: 927,
					height: 647,
			...
			getDetailInfo(){
				...
				this.$axios.get('/index/getVideoInfo',
            {
              id: query.themeid,
              commentId: query.id,
              type: query.type,
