<template>
	<div class="phone-play">
		...
		<van-dialog v-model="showDialog" title="观看密码" :beforeClose="checkPw">
			<p style="font-size: 12px;color: red;text-align: center;">{{errorInfo}}</p>
			<van-cell-group inset>
				<van-field v-model="password" label="" placeholder="请输入密码" />
			</van-cell-group>
		</van-dialog>
				...
				this.$axios.get('/index/getVideoInfo', {id: query.themeid, type: query.type}).then(res => {
					...
					this.$nextTick(() => {
						...
						if (this.detailInfo.themeStatus === 2 && this.$route.query.type === '0') {
							...
							// 	// playUrl = `${playUrl.split('://')[0]}://${playUrl.split('//')[1].replace('8085','8084')}`;
							// }
