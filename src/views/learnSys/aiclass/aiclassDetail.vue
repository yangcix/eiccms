<template>
	<div class="second-wrap">
		...
		<div class="content-wrap scroll-wrap" v-if='activeIndex == 0'>
          ...
          <li v-if="detailInfo.resources == 1">
						...
						<p class="value">{{detailInfo.grindingSchoolName + ' - ' + detailInfo.grindingBuildingName + ' - ' + detailInfo.grindingClassroomName}}</p>
					</li>
					<li>
						<p class="title">状态：</p>
              ...
              <div v-for="(item, index) in detailInfo.teachingFileList" :key="index">
                <span>{{ item.name }}</span><span @click="handleDownload(item)" style="margin-left: 10px;color: #1e80ff;cursor: pointer;">下载</span>
              </div>
            </div>
            <span v-else class="value">无</span>
				...
				<div  v-if="detailInfo.type ==2">
					...
					<div class="btn-wrap" v-if="permission.report && detailInfo.aiConfigId == 2 && detailInfo.aiStatus == 3">
						<el-button type="primary" v-if="detailInfo.isResetting == 0" @click="resetting()">重置分析</el-button>
						<el-button style="color:#b3b6ba" type="primary" v-if="detailInfo.isResetting == 1" plain disabled>重置分析</el-button>
					</div>
				</div>
			</div>
    ...
    <el-dialog :title="detailInfo.name + (detailInfo.type == 1 ? '(磨课中)': '(已结束)')" :close-on-click-modal="false" :visible.sync="alertVideo" width="600px" class="videoBox"
