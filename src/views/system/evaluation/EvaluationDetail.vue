<template>
	<div class="second-wrap">
        ...
        <div class="total">
          <div class="block-show">
            ...
            <div class="right-show" @click="toEvaluate">
              <p><span>{{detailInfo.commented}}</span><span>详情</span></p>
              <p>已评课人数</p>
            </div>
          </div>
          <div class="block-show">
            ...
            <div class="right-show" @click="toEvaluateUn">
              <p><span>{{detailInfo.uncomment}}</span><span>详情</span></p>
              <p>未评课人数</p>
            </div>
          </div>
          <div class="block-show">
            ...
            <div class="right-show" @click="toReport">
              ...
              <p><span>{{detailInfo.reportTotal}}</span><span>详情</span></p>
              <p>报告数量</p>
            </div>
          </div>
        </div>
        ...
        <li>
          ...
          <p v-if="detailInfo.status == 6" class="value">(驳回理由：{{detailInfo.reason ? detailInfo.reason : '无理由'}})</p>
				</li>
        <li v-if="detailInfo.evaluationType == 2">
					<p class="title">教案：</p>
          <div v-if="detailInfo?.teachingFileList?.length > 0">
            <div v-for="(item, index) in detailInfo.teachingFileList" :key="index">
              <span>{{ item.name }}</span><span @click="downFile(item)" style="margin-left: 10px;color: #1e80ff;cursor: pointer;">下载</span>
            </div>
          </div>
          <span v-else class="value">无</span>
        </li>
        <div v-if="detailInfo.resources == 3 && (detailInfo.status == 1 || detailInfo.status == 2 || detailInfo.status == 6)">

        </div>
        <div v-else>
          <div v-if="detailInfo.resources == 1 || detailInfo.resources == 2" class="data-titile" style="margin-top: 10px;">{{ detailInfo.type == 1 ? '视频资源：' : '直播资源：' }}</div>
      ...
      <el-dialog
        ...
        <div class="evaluation-title">{{evaluationReport.name}}总报告</div>
          <div class="evaluation-content">
            <el-table v-if="evaluationReport.strategy == 1 && evaluationReport.wordDataDtos.length > 0" :data="evaluationReport.wordDataDtos" style="width: 100%;" border default-expand-all>
            <el-table-column width="320" align="left" prop="levelOne" label="评价要素"> </el-table-column>
            <el-table-column v-if="evaluationReport.itemList[0] && (evaluationReport.itemList[0].type == 0 || evaluationReport.itemList[0].type == 2)" align="center" label="分值">
            ...
            <el-table-column v-if="evaluationReport.itemList[0] && (evaluationReport.itemList[0].type == 0 || evaluationReport.itemList[0].type == 2) " align="center" label="平均分">
              <template slot-scope="scope">
                <span>
            ...
            <el-table-column  v-if="evaluationReport.itemList[0] && (evaluationReport.itemList[0].type == 1 || evaluationReport.itemList[0].type == 2)" align="center" label="评语">
              <template slot-scope="scope">
            ...
            <el-table-column v-if="evaluationReport.isOpenRemarks == 1" align="center" :label="evaluationReport.remarksName">
              <template slot-scope="scope">
                <span>
          ...
          <el-table v-if="evaluationReport.strategy == 2 && evaluationReport.wordDataDtos.length > 0" :data="evaluationReport.wordDataDtos" style="width: 100%;" border default-expand-all :span-method="(param)=>objectSpanMethod(param,evaluationReport.wordDataDtos)">
          ...
          <el-table v-if="evaluationReport.strategy == 3 && evaluationReport.wordDataDtos.length > 0" :data="evaluationReport.wordDataDtos" style="width: 100%;" border default-expand-all :header-cell-style="handerMethod" :span-method="(param)=>objectSpanMethod(param,evaluationReport.wordDataDtos)">
      ...
      <el-dialog
        ...
        <div class="evaluation-title">{{detailInfo.name}}在线评课</div>
          ...
          <el-tab-pane v-for="(item, index) in evaluationList" :key="index" :label="item.name" :name="index.toString()">
          <div class="evaluation-content">
            ...
            <el-table-column v-if="item.itemList[0] && (item.itemList[0].type == 0 || item.itemList[0].type == 2)" align="center" label="评分">
              <!-- <template slot-scope="scope">
                <div v-if="scope.row.type == 0">
                  <el-input-number class="width-6" v-model="scope.row.userScore" clearable :min="1" :max="scope.row.score"></el-input-number>
                </div>
            ...
            <el-table-column v-if="item.itemList[0] && (item.itemList[0].type == 1 || item.itemList[0].type == 2)" align="center" label="评语">
              <!-- <template slot-scope="scope">
                <div v-if="scope.row.type == 0">
                  <el-input-number class="width-6" v-model="scope.row.userScore" clearable :min="1" :max="scope.row.score"></el-input-number>
                </div>
          ...
          <el-table v-if="item.strategy == 3 && item.wordDataDtos.length > 0" :data="item.wordDataDtos" style="width: 100%;" border default-expand-all :header-cell-style="handerMethod" :span-method="(param)=>objectSpanMethod(param,item.wordDataDtos)">
            <el-table-column width="100" align="center" prop="levelOne" label="一级名称"> </el-table-column>
            <el-table-column width="100" align="center" prop="levelTwo" label="评价项目"> </el-table-column>
            <el-table-column width="320" align="left" prop="levelThree" label="评价要素"> </el-table-column>
            <el-table-column v-if="item.itemList[0] && (item.itemList[0].type == 0 || item.itemList[0].type == 2)" align="center" label="分值">
      ...
      <el-dialog
        ...
        <div>
          ...
          <el-input style="margin-left: 20px;" class="width-2" v-model="userKey" clearable placeholder="请输入用户姓名关键字" v-search="searchLive"></el-input>
        ...
        <div>
          <el-input class="width-2" v-model="userKey" clearable placeholder="请输入用户姓名关键字" v-search="unsearchLive"></el-input>
      ...
      <el-dialog
        ...
        <div>
          <p>人工评课报告</p>
          <div v-for="(item, index) in reportList" :key="index" style="display: flex;justify-content: space-between;align-items: center;padding: 0 20px;">
          ...
          <div v-if="reportAiList.length > 0">
            <p>AI评课报告</p>
            <div v-for="(item, index) in reportAiList" :key="index" style="display: flex;justify-content: space-between;align-items: center;padding: 0 20px;">
              <span style="width: 300px;">{{ item.name }}</span>
              <!-- <el-button type="text" @click="downloadReport(1, item)">下载</el-button> -->
              <el-button v-if="detailInfo.aiStatus != 3" type="text" @click="viewReport(1, item)">查看复盘/报告</el-button>
              <el-button v-if="detailInfo.isResetting == 0 && detailInfo.aiConfigId == 2 && detailInfo.aiStatus == 3" type="text" @click="resetting()">重置分析</el-button>
              <el-button style="color: #b3b6ba" v-if="detailInfo.isResetting == 1 && detailInfo.aiConfigId == 2 && detailInfo.aiStatus == 3" type="text" disabled>重置分析</el-button>
    ...
    //重置分析弹窗
      ...
      downloadReport(type, val){
        if(type == 0){
          ...
          this.$axios.get('/sm/comment/exportCommentReportWord',{commentId: val.id}, 'blob').then(res => {
      ...
      async viewTem () {
        ...
        for(let i = 0;i < this.detailInfo.templatesList.length; i++) {
          let obj = await this.$axios.get('/sm/template/selectItemByTemplate',{templateId:this.detailInfo.templatesList[i].id})
    ...
    flitterData(arr) {
      ...
      toReport(){
        ...
        if(this.detailInfo.reportTotal > 0 ){
          this.$axios.get('/sm/comment/report', {
      ...
      pageChangeUn(val){
          ...
          this.unevaluation();
			...
			handleExportDataList(item){
				...
				axios.get('/sm/comment/exportCommentReportTwo', {
					params: data,
					responseType: 'blob'
				}).then(res => {
