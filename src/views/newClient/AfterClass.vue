<template>
	<div style="padding: 40px 0;background-color: white;min-height: 694px;">
      ...
      <p class="home-title" @click="setTitle(2)">
				<!-- <i class="iconfont iconnav-vedio"></i> -->
				<span :class="actives == 2 ? 'actives' : ''">课外知识</span>
        <svg :class="actives == 1 ? 'acts' : ''" width="30" height="10" viewBox="0 0 30 10" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <stop offset="1" stop-color="#2F60F6"/>
			...
			<div class="select-wrap" v-if="release === 1">
				<ul>
					<li :class="activeIndex == 0 ? 'active' : ''" @click="changeLabel(0)"><p class="parent-title">全部</p></li>
					<li v-for="(val, index) in labelList" :key="index" @click="changeLabel(val.id)" :class="activeIndex == val.id ? 'active' : ''">
			...
			<VideoList v-if="actives == 1" :videoList="liveList" type="0"></VideoList>
      <VideoList v-else :videoList="liveList" type="2"></VideoList>
...
<script>
	...
	import VideoList from './components/VideoListss'
	import {creatTree, session} from "../../assets/js/utils";
	export default {
		name: '',
		data() {
			return {
        actives: this.$route.query.id ? this.$route.query.id : 1,
				liveList: [],
				labelList: [],
				activeIndex: '',
				pageNum: 1,
				...
				isSelect: false,
				release: 2,
				categoryId: (this.$route.query.id && this.$route.query.id == 2) ? 6 : 2
			}
		},
      ...
      getVideolist() {
        ...
        this.$axios.post('/index/getVideo', params).then(res => {
          this.liveList = res.data.pageList;
			...
			getType() {
				this.$axios.get('/index/getLabelInfo', { categoryId: this.categoryId }).then(res => {
