<template>
  <div class="second-wrap">
    <p class="content-title">
      <el-page-header
          @back="goBack"
          :content="`${$route.query.id ? '编辑' : '新增'}大单元分析`"
      ></el-page-header>
    </p>
    <div class="content-wrap">
      <div class="item-scroll">
        ...
        <div class="item-scroll-line">
          <div class="item-wrap">
            ...
            <el-button style="margin-right: 20px" v-if="addEditInfo.largeUnitClassList.length > 0" @click="clearAlert()">
              清空选择
            </el-button>
            <el-button style="margin-right: 20px; background-color: #b3b6ba;color: #ffffff;" v-else disabled>清空选择
            </el-button>
                ...
                <template slot-scope="scope">
                  <div class="iconList">
                    <span style="margin-right: 10px">
                      <i class="iconfont icon-ketuozhuai" style="color: rgb(0, 116, 208)" @click="rowDrop()"></i></span>
                    <span>
                      <i class="iconfont icon-shanchu1" style="color: rgb(255, 81, 64)"  @click="deleteData(scope.row)"></i>
                    </span>
                  </div>
                </template>
              </el-table-column>
      ...
      <div class="item-scroll-bottom">
        <el-button type="primary" class="save-btn" @click="!$route.query.id ? save() : update()">确认</el-button>
      </div>
    </div>
      ...
      <div class="dialog-scrollTo">
        ...
        <div class="table-list">
          <el-table
            ...
            <el-table-column align="left" type="selection" reserve-selection width="50">
            </el-table-column>
            <el-table-column
                align="left"
                prop="courseName"
            ...
            <el-table-column align="left" prop="gradeName" label="年级">
            </el-table-column>
            <el-table-column
                align="left"
                prop="classTime"
        ...
        <div class="dialog-btn">
          <el-button
              type="primary"
              @click="confirm()"
              style="margin-left: 20px; margin-right: 15px">确定
          </el-button>
...
export default {
  ...
  methods: {
    ...
    // 课堂管理列表数据
    getClassroom() {
      this.$axios.get('/aiLargeUnit/listClassroom', {
          ...
          if (this.selectRowKeys.length > 0) {
            this.classroomList.forEach(item => {
              ...
              if (this.selectClassroomList.length < this.selectRowKeys.length) {
                const selectIdMap  = new Map(this.selectClassroomList.map(item => [item.id, item]));
    ...
    confirm() {
      ...
      this.selectClassroomList.forEach(item => {
        const unitClassAddIdMap = new Map(this.addEditInfo.largeUnitClassList.map(unitClassItem => [unitClassItem.id, unitClassItem ]));
    ...
    // 课堂数据删除
    deleteData(row) {
      const index = this.addEditInfo.largeUnitClassList.findIndex(item => item.id === row.id && item.type === row.type);
      if (index !== -1) {
    ...
    verify() {
      ...
      if (
          this.addEditInfo.largeUnitClassList.length < 2 ||
          this.addEditInfo.largeUnitClassList.length > 21
      ) {
        this.$message("可分析课堂节数，仅可在2-20节课之间！", "error");
        return true;
      }
