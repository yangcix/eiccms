	...
	<div class="content-main-wrap">
		...
		<div class="main-wrap">
			...
			<div class="search-operat">
				<div>
					<el-input v-model="searchKey" class="width-2" placeholder="请输入角色名称关键字进行查询" v-search="search" clearable></el-input>
					...
					<el-table-column
						...
						<template slot-scope="scope">
							<ul class="operat-list" v-if="isAdmin == 'true'">
								...
								<li @click="roleAuthEdit(scope.row)" v-if="permission.updateRoleOptAuth && scope.row.name !== '学生'">权限授权</li>
								<li @click="deleteData(scope.row)" style="color:#f56c6c" v-if="permission.delete && scope.row.type != 0">删除</li>
							</ul>
							<ul class="operat-list" v-else>
								...
								<li @click="roleAuthEdit(scope.row)" v-if="permission.updateRoleOptAuth && scope.row.type != 0">权限授权</li> -->
								<li @click="add(0, scope.row)" v-if="permission.update && scope.row.type != 0">编辑</li>
								<li @click="roleAuthEdit(scope.row)" v-if="permission.updateRoleOptAuth && scope.row.name !== '学生' &&!(scope.row.sign=='admin'||scope.row.sign=='super'||scope.row.sign=='teacher'||scope.row.sign=='student'||scope.row.sign=='charge')">权限授权</li>
								<li @click="deleteData(scope.row)" style="color:#f56c6c" v-if="permission.delete && scope.row.type != 0">删除</li>
							</ul>
						</template>
					</el-table-column>
		...
		<el-dialog
			...
			<div class="dialog-wrap">
				<div class="dialog-item">
					<el-cascader v-model="authEditInfo.menuIds"
	...
	export default {
		...
		methods: {
			...
			authEditConfirm(){
				...
				data = [...new Set(data)];
				this.$axios.post('/sys/role/updateRoleOptAuth', {menuIds: data, roleId: this.authEditInfo.roleId}).then(res => {
					if(res.code == 200){
						this.authShow = false;
						this.$message(res.message, 'success');
						this.getRoleList();
