			...
			<div class="content">
				...
				<div class="content-info">
					<div class="class-info">
						...
						<p v-if="loginShow == false && themeInfo" class="class-name">{{themeInfo.habitusName + '第' + themeInfo.numCount + '课'}}</p>
						<p v-else></p>
					</div>
	...
	export default {
		...
		methods: {
			...
			checkUser() {
				...
				this.$axios.post(`/sm/interactive/checkUser`, data).then(res => {
					...
					if(res.data.termianlist) {
						...
						this.oldTermianlist = res.data.termianlist;
			...
			checkSign(){
				this.getAudioVideo({audio:true,video:true}).then(res=>{
						...
						if(this.userType == 1){ //主讲
							this.$router.push(`/normalmainroom?id=${this.$route.query.id}&name=${this.themeInfo.habitusName + '第' + this.themeInfo.numCount + '课'}&room=${this.classroomId}&isLocal=1&url=${this.judgeSignalUrl}&cacheTime=${this.dvConfig.cacheTime}&maxBandwidth=${this.dvConfig.maxBandwidth}&minBandwidth=${this.dvConfig.minBandwidth}&maxFrameRate=${this.dvConfig.maxFrameRate}&minFrameRate=${this.dvConfig.minFrameRate}`);
						}else{
							this.$router.push(`/normalassistantroom?id=${this.$route.query.id}&name=${this.themeInfo.habitusName + '第' + this.themeInfo.numCount + '课'}&room=${this.classroomId}&isLocal=1&url=${this.judgeSignalUrl}&cacheTime=${this.dvConfig.cacheTime}&maxBandwidth=${this.dvConfig.maxBandwidth}&minBandwidth=${this.dvConfig.minBandwidth}&maxFrameRate=${this.dvConfig.maxFrameRate}&minFrameRate=${this.dvConfig.minFrameRate}`);
						}
          }).catch(err=>{
			...
			handleIdentityIn(item) {
				...
				if (item.id === 2) { // 观看身份
					this.$router.push(`/normalassistantroom?id=${this.$route.query.id}&name=${this.themeInfo.habitusName + '第' + this.themeInfo.numCount + '课'}&isBystanderIn=1`);
					return;
				}
				if(item.id === 0){
				...
				if (item.id === 2) { // 观看身份
					this.$axios.get("/sys/crossSchoolInteraction/getInteractionInfo", { id: this.$route.query.id }).then(res => {
						...
						if (res.code === 200) {
							...
							this.$router.push("/normalassistantroom?id=" + this.$route.query.id + '&type=2&isOut=' + this.isOut + '&mainId=' + res.data.mainId);
						}
					});
					// this.$router.push('/normalassistantroom?id=' + this.$route.query.id + '&room=' + this.classroomId + '&type=' + this.thisIdentity + '&mainId=' + this.mainId);
					return;
				}
				this.thisIdentity = item.id;
			},
        ...
        if (this.$route.query.type) {
          this.$router.push({ path: "/normalInteractlogin", query: { id: this.$route.query.id, type: 1, themeid: this.$route.query.themeid } });
        } else {
          this.$router.push({ path: "/normalInteractlogin", query: { id: this.$route.query.id, themeid: this.$route.query.themeid } });
        }
			},
			//获取平台logo和名字
			getLogo(){
        ...
        window.open(routeUrl.href, '_blank');
			...
			inclass(){
				this.$axios.get('/sm/interactive/info',{ id: this.$route.query.id }).then(res => {
					...
					if(type == 0){
						this.$router.push({path:'/normalmainroom', query: { themeid: this.$route.query.themeid,id: this.$route.query.id}})
					}else if(type == 1){
						this.$router.push({path:'/normalassistantroom', query: { themeid: this.$route.query.themeid, id: this.$route.query.id}})
					}else{
						if(this.code == ''){
						...
						this.$axios.get('/sm/interactive/auditPassWord', { id: this.$route.query.id, auditPassWord: this.code}).then(res => {
							if(res.data == true) {
								this.$router.push({path:'/normalassistantroom', query: {themeid: this.$route.query.themeid, id: this.$route.query.id, isBystanderIn: 1}})
							}else {
			...
			localInPush(){
				if(this.role == 1){ //主讲
					// this.$router.push(`/normalmainroom?id=${this.$route.query.id}&room=${this.classroomId}&isLocalSign=1&cacheTime=${this.dvConfig.cacheTime}&maxBandwidth=${this.dvConfig.maxBandwidth}&minBandwidth=${this.dvConfig.minBandwidth}&maxFrameRate=${this.dvConfig.maxFrameRate}&minFrameRate=${this.dvConfig.minFrameRate}`);
					this.$router.push('newRoom')
				}else{
					this.$router.push(`/normalassistantroom?id=${this.$route.query.id}&room=${this.classroomId}&isLocalSign=1&cacheTime=${this.dvConfig.cacheTime}&maxBandwidth=${this.dvConfig.maxBandwidth}&minBandwidth=${this.dvConfig.minBandwidth}&maxFrameRate=${this.dvConfig.maxFrameRate}&minFrameRate=${this.dvConfig.minFrameRate}`);
				}
			},
			// 校外本机跳转
			localOutPush() {
				if(this.thisIdentity == 1){ //主讲
					this.$router.push(`/normalmainroom?id=${this.$route.query.id}&room=${this.classroomId}&type=${this.thisIdentity}&mainId=${this.mainId}&isLocalSign=1&cacheTime=${this.dvConfig.cacheTime}&maxBandwidth=${this.dvConfig.maxBandwidth}&minBandwidth=${this.dvConfig.minBandwidth}&maxFrameRate=${this.dvConfig.maxFrameRate}&minFrameRate=${this.dvConfig.minFrameRate}`);
				}else{
			...
			judgeSignal(){
				...
				this.$axios.get(url, {url: this.judgeSignalUrl}).then(res => {
					if(res.code == 200 && res.data){
						...
						this.$axios.get('/sm/interactive/deleteTerminal', {id: id}).then(res => {})
						this.verity = 0;
					}
				});
			},
