    ...
    <div class="main-wrap">
      ...
      <div class="table-list">
        <el-table
          ...
          style="width: 100%;"
        >
          <el-table-column align="center" prop="name" label="大单元分析名称">
          </el-table-column>
...
<!--                <el-button
                  style="color: #b3b6ba"
                  type="text"
                  v-if="scope.row.status === 0 /* && permission.downloadOrResetting */"
                  >重置分析</el-button
                >-->
              </div>
            </template>
...
export default {
  ...
  methods: {
    ...
    // 查看报告
    openReport(val) {
      this.$axios.get('/aiLargeUnit/report',{params: {id: val.id},responseType: 'blob'},'blob').then(res => {
    ...
    // 下载压缩包
    download(val) {
      this.$axios.get('/aiLargeUnit/download',{params: {id: val.id},responseType: 'blob'},'blob').then(res => {
    ...
    handleGetData() {
      this.loading = true;
      this.$axios.get("/aiLargeUnit/list", {
