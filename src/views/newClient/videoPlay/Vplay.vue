...
export default {
	...
	methods: {
        ...
        return;
			...
			this.$axios.post('/index/checkCommentAuth', { type: 1, objectId: this.themeId, userId: this.userId}).then(res => {
				if (res.code === 200) {
					this.isBindEvaluation = true;
					this.commentData = res.data;
					this.$axios.post('/index/getUserCommentDetail', { type: 1, objectId: this.themeId }).then(res => {
			...
			if (this.commentData.status !== 1) {
                ...
                this.$message(res.message,'error')
						...
						if (this.commentData.status !== 1) {
							...
							this.$axios.post('/index/checkCommentAuth', { type: 1, objectId: this.themeId, userId: this.userId}).then(ress => {
								if (ress.code == 200) {
									this.activeName = '0';
