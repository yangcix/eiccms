<template>
    <div class="content-main-wrap" v-if="aiType == 2 && aiOpenStatus == 1 && feeModel == 1 && (userId === 1 || userId === 2)">
      <p class="content-title">AI分析次数充值</p>
      <div class="main-wrap" style="margin-left: 2px;">
        <template>
            <div class="introduce">
                <span style="margin-right: 15px;font-weight: bold">AI课堂分析剩余次数：{{ count + '次' }}</span> <span style="font-weight: bold">大单元分析次数: {{ largeUnitCount + '次' }}</span>
                <el-button type="primary" class="content" @click="AIRechargeShow = true" v-if="userId === 1">充值</el-button>
          </div>
        </template>
        <el-dialog class="recharge"
			title="AI分析次数充值"
			:close-on-click-modal="false"
			:visible.sync="AIRechargeShow"
			width="500px"
      @close="cancel()">
          <div class="popUp"><em class="custom">*</em><span class="span-title">充值产品</span>
          ...
          <div class="popUp"><em class="custom">*</em><span class="span-title">充值次数</span>
            <el-input class="input" v-model="number" placeholder="输入充值次数" @input="(v) => (number = v.replace(/[^0-9]/g,''))" @blur="defocus()"/>
        ...
        <div class="data-table table-border">
          <el-table v-loading="loading" :data="tableData" style="width: 100%;min-width: 900px;">
            <el-table-column prop="products" align="center" label="充值产品">
            </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        : "",
        aiType: JSON.parse(localStorage.getItem("sysInfo")).aiConfigId ? JSON.parse(localStorage.getItem("sysInfo")).aiConfigId : 1,
        aiOpenStatus: JSON.parse(localStorage.getItem("sysInfo")) ? JSON.parse(localStorage.getItem("sysInfo")).aiOpenStatus : 0,
        feeModel: JSON.parse(localStorage.getItem("sysInfo")) ? JSON.parse(localStorage.getItem("sysInfo")).type : 1,
        products: 1,
        number: '',
        count: 0,
        largeUnitCount: 0,
