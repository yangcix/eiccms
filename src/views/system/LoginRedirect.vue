...
<!--      <p v-if="success">Login successful! Redirecting...</p>-->
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    console.log("MD5: ", MD5(router.password))
    const params = {
      loginType: "web",
      account: router.account,
      password: MD5(router.password),
      remember: false,
      type: 1
    };
    this.$axios.post('/auth/login', params).then(res => {
      if (res.code === 200) {
        ...
        if (router.type === "1") {
          ...
          this.changeMenu();
          this.$router.push("/aiGrinding?loginRedirect=1"); // 磨课
        }
        // else if(router.type === "2"){
        //   console.log("2222222222222222222")
        //   this.$router.push("/sm/aiclassAddEdit?type=2&mode=1"); // 新增磨课
        // }
      }
    });
  },
