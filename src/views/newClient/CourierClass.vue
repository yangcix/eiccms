	...
	<div style="padding: 40px 0;background-color: white;min-height: 694px;">
        ...
        <svg width="30" height="10" viewBox="0 0 30 10" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <stop offset="1" stop-color="#2F60F6"/>
			...
			<VideoList :videoList="liveList" type="0" isCheck='1'></VideoList>
...
<script>
	...
	import VideoList from './components/VideoList1'
	...
	export default {
		...
		methods: {
			...
			// },
			handleType() {
				this.$axios.get('/index/getLabelInfo', { categoryId: 5 }).then(res => {
					this.firstLabelList = creatTree(res.data);
					this.firstLabelList.forEach(v => {
						if (v.children) {
							v.children.unshift({ name: '全部', id: '' })
			...
			getLivelist(){
				...
				console.log(this.indexes.toString(this.indexes),'idsss',this.indexes.toString(this.indexes).split(','))
				if (idList.length >= 1) {
					params.labelIds = this.release === 1 ? labelIdArr : this.indexes.toString();
				}
