	...
	<div class="liveInteract">
		<div class="video-content-wrap">
			<div class="video-wrap">
				...
				<div class="video-tag-info">
					<div class="video-tag-left">
						<ul v-if="detailInfo.labels">
							<li v-for="(val, index) in detailInfo.labels" :key="index">{{ val }}</li>
						</ul>
				...
				<el-dialog class="dialogPassword" :modal="false" :show-close="false" :visible.sync="dialogVisible" width="450px"
					:close-on-click-modal="false" style="position: absolute;" :z-index="1997">
					<p style="text-align: center;width: 100%;;font-size: 14px;color: red;position: absolute;top: 20px;left: 0;">{{
						passwordMsg
					}}</p>
					<div class="admin">
						<div class="name">观看密码<label>*</label>：</div>
						<el-input class="width-5" placeholder="请输入观看密码" show-password clearable v-model="password">
						</el-input>
					</div>
					...
					<div class="chat-box" ref="chatBox" id="chatBox">
						<div v-for="( item, index) in inList" :key="index" class="message">
							<div class="message-author">
								<img src="@/assets/imgs/teacherIcon.png" alt="" v-if="item.userName == detailInfo.nickName"
									class="avatar">
								<img src="@/assets/imgs/studentIcon.png" alt="" v-else class="avatar">
								<span>{{ item.userName }}</span> <span class="teacherMark"
									v-if="item.userName == detailInfo.nickName">教师</span>
							</div>
							<p v-html="item.message" class="a-message" @click="imgB(index)"></p>
						</div>
					</div>
					<div id="editor"></div>
					<el-button type="primary" @click="send" class="send-btn" :disabled="!isLogin || forbidden">发送</el-button>
					<div class="forbidden" v-if="forbidden">全员禁言</div>
					<div class="unLogin notselect" v-if="!isLogin">
						<span style="color:#196EF6;cursor: pointer;" @click="toLogin">登录</span>后，可参与互动讨论
					</div>
					<transition name="fade">
						<div class="newTips" v-if="toNew" @click="clickNew">有{{ num }}条新消息</div>
					</transition>
				</div>
		...
		<div class="imgViewBox" v-if="centerDialogVisible" @click="centerDialogVisible = false">
					...
					<el-tab-pane v-for="(item, index) in evaluationList" style="pointer-events: auto;" :key="index"
								...
								<el-table-column v-if="item.itemList[0] && (item.itemList[0].type == 0 || item.itemList[0].type == 2)" align="center" label="评分"
									...
									<template slot-scope="scope">
										<el-input-number v-if="!scope.row.children || scope.row.children.length == 0" class="width-6"
											v-model="scope.row.userScore" :precision="0" clearable :min="0" @blur="blurNumber"
											@change="changeNum" :max="scope.row.score"></el-input-number>
									</template>
								</el-table-column>
                        ...
                        (scope.row.type == 0 || scope.row.type == 2) && (!scope.row.children || scope.row.children.length == 0) ? scope.row.score
							...
							<el-table v-if="item.strategy == 3 && item.wordDataDtos.length > 0" :data="item.wordDataDtos"
								:header-cell-style="handerMethod" style="width: 100%;;" border
		...
		<el-dialog title="提交评课" append-to-body :close-on-click-modal="false" :visible.sync="checkShow" width="420px" :z-index="2000">
	...
	methods: {
		...
		visity() {
			...
			this.settime = setTimeout(() => {
				let data = {
					pageName:  this.recordVal(this.detailInfo.categoryId),
					contentType: 1
				}
				this.$axios.post('/buried/point/record', data).then(res => {
					console.log('记录一次', res)
				});
			}, 300000);
		},
		visityv() {
			console.log('正在记录智能研修')
			if(this.settimev){
				clearTimeout(this.settimev);
			}
			this.settimev = setTimeout(() => {
				let data = {
					pageName: this.recordVal(1),
					contentType: 1
				}
				this.$axios.post('/buried/point/record', data).then(res => {
      ...
      this.$axios.post("/index/commentOnline", evaluationData).then(
		...
		submitData() {
			this.$axios.post('/index/commentOnline', this.evaluationData).then(res => {
					if (res.code == 200) {
		...
		imgB(index) {
			...
			this.$nextTick(() => {
				...
				if (img.clientWidth > img.clientHeight) {
					let h = img.clientHeight * this.$refs.block.clientWidth / img.clientWidth
					if (h > this.$refs.block.clientHeight) {
						this.$refs.imgView.style.height = this.$refs.block.clientHeight + 'px'
						let w = this.$refs.block.clientHeight * img.clientWidth / img.clientHeight
						this.$refs.imgView.style.width = w + 'px'
					} else {
						this.$refs.imgView.style.width = this.$refs.block.clientWidth + 'px'
		...
		editorInit() {
					...
					_that.$nextTick(() => {
						...
						id.onchange = function (e) {
							...
							reader.onload = function (ev) {
								...
								_that.editor.cmd.do('insertHTML', `<img src=${imgBase64Data} style="width:200px" class="sendImg">`)
							}
							id.value = ''
						}
					})
			...
			this.$nextTick(() => {
				...
				editerInput.onkeydown = (e) => { // 回车提交表单
					let event = e || window.event
		...
		send() {
			...
			str = str.replace(/<br>/g, '')
			let messageNUm = (this.editor.txt.html().match(/<img[^>]+>/g) ? this.editor.txt.html().match(/<img[^>]+>/g).length : 0) + str.length
		...
		handleEnsure() {
			...
			// const params = {
			//   password: this.password,
			//   watchId: this.detailsData.watchId
			// };
			// this.$http.apiWatchWatchurl(params).then(res => {
			//   if (res.code === 200 && res.success) {
			//     this.dialogVisible = false;
			//     this.playUrl = res.data.playUrl;
			//     // this.$refs.video.innerHTML = res.data.iframe;
			//     sessionStorage.setItem("play", res.data.playUrl);
			//   }
			// });
		},
		//获取详情
      ...
      if(query.etype){
		...
		setInit() {
			...
			console.log(this.detailInfo.themeStatus,'3333',this.$route.query.type)
			...
			this.$nextTick(() => {
				...
				if (this.detailInfo.themeStatus == 2 && this.$route.query.type == '0') {
					...
					console.log('this.detailInfo.recode',this.detailInfo.recode)
					if(this.detailInfo.recode == 1){
						if(this.$route.query.etype){
          ...
          if (divs) {
            document.querySelector("#videoWrap").innerHTML = videoEl;
					...
					this.startVideo();
					// this.checkFLVPlayback(this.playUrl);
					console.log('可播放');
				}
			})
		},
		checkFLVPlayback(url) {
			fetch(url, { method: 'GET' })
				.then(response => {
					if (response.ok && response.headers.get('content-type') === 'video/x-flv') {
						console.log('flv可以播')
						this.startVideo();
					} else {
						console.log('flv错误')
