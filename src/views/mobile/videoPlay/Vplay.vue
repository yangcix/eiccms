...
<script>
	...
	export default {
		...
		methods: {
			...
			//初始化视频
			initVideo(){
				// let url =  'http://192.168.164.59' + (this.videoList[0].url);
				let url = 'http://' +	location.hostname + (this.videoList[2].url);
				console.log(url,'21212')
				let options = {
					id: 'myVideo',
					// url: location.hostname + (this.videoList[0].url),
					url: url,
					autoplay: true,
					videoInit: true,
