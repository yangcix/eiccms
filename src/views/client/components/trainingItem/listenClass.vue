			...
			goDetail(val){
				...
				if(val.type == 0){
					...
					if(val.liveStatus == 1 || val.liveStatus == 0){
						window.open('/hplay?themeid='+ val.objectId +'&type=0&vtype=1' + (val.resources == 2 ? '&etype=1' : ''), '_blank')
					}
					if(val.liveStatus == 2){
						window.open('/pplay?themeid='+ val.objectId +'&type=0&vtype=1' + (val.resources == 2 ? '&etype=0' : ''), '_blank')
					}
				}else{
					if(val.resources == 3 || val.resources == 4){
						window.open('/Hplays?themeid='+ val.id, '_blank')
					}else{
						window.open('/vplay?themeid='+ val.objectId +'&type=2&vtype=1', '_blank')
					}
				}
